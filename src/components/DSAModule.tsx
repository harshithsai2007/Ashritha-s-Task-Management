...
export default function DSAModule({ logs, onAddLog, onUpdateLog, onDeleteLog }: DSAModuleProps) {
  ...
  const handleToggleComplete = (log: DSALog) => {
    const nextVal = !log.isCompleted;
    onUpdateLog({
      ...log,
      isCompleted: nextVal,
  ...
  const activeTasks = logs.filter((l) => !l.isCompleted);
  const completedTasks = logs.filter((l) => l.isCompleted);

  return (
    <div className="space-y-6" id="dsa-module-panel">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-900/40 p-6 rounded-2xl border border-slate-800">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white font-semibold text-xs px-4 py-2.5 rounded-xl transition-all font-mono hover:scale-[1.02] shadow-md shadow-purple-600/10"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-[#111827] rounded-[24px] p-6 border border-white/5 space-y-4">
            <span className="text-[10px] font-mono font-bold tracking-widest text-slate-500 uppercase">
              ACTIVE ALGORITHMS ({activeTasks.length})
            </span>
            <div className="space-y-2.5">
              <AnimatePresence mode="popLayout">
                {activeTasks.length === 0 ? (
                  <p className="text-xs text-slate-650 font-medium">All algorithms mastered. Incredible!</p>
                ) : (
                  activeTasks.map((task) => (
                    <motion.div
                      key={task.id}
                      layoutId={`dsa-${task.id}`}
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <button
                          ...
                          className="w-5 h-5 rounded-md border border-slate-800 hover:border-purple-500/80 cursor-pointer flex items-center justify-center transition-colors shrink-0"
                        >
                          <div className="w-3 h-3 rounded bg-transparent group-hover:bg-purple-600/30 transition-colors" />
                        </button>
          ...
          <div className="bg-[#111827]/60 rounded-[24px] p-6 border border-white/[0.02] space-y-4">
            <span className="text-[10px] font-mono font-bold tracking-widest text-slate-500 uppercase">
              MASTERED PROBLEMS ({completedTasks.length})
            </span>
            <div className="space-y-2.5">
              <AnimatePresence mode="popLayout">
                {completedTasks.length === 0 ? (
                  <p className="text-xs text-slate-650 font-medium">No algorithms completed yet.</p>
                ) : (
                  completedTasks.map((task) => (
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <button
                          onClick={() => handleToggleComplete(task)}
                          className="w-5 h-5 rounded-md bg-purple-600/20 border border-purple-500/40 cursor-pointer flex items-center justify-center shrink-0"
                        >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-900 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-purple-500/80 transition-all font-sans"
                  />
                </div>
                ...
                <div className="flex gap-2.5 pt-2">
                  ...
                  <button
                    type="submit"
                    className="flex-1 bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2.5 rounded-xl text-xs transition-colors font-mono shadow-md shadow-purple-600/10"
                  >
                    COMMIT
                  </button>
                </div>
