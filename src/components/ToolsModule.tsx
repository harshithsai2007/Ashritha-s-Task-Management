...
import { FIToolDay } from "../types";
import { Wand2, Plus, X, Trash2, CheckCircle2 } from "lucide-react";
...
export default function ToolsModule({ days, onUpdateDay, onAddDay, onDeleteDay }: ToolsModuleProps) {
  ...
  const handleToggleComplete = (day: FIToolDay) => {
    const nextVal = !day.isCompleted;
    onUpdateDay({
      ...day,
      isCompleted: nextVal,
  ...
  const activeTasks = days.filter((d) => !d.isCompleted);
  const completedTasks = days.filter((d) => d.isCompleted);

  return (
    <div className="space-y-6" id="tools-module-panel">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-900/40 p-6 rounded-2xl border border-slate-800">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-amber-600 hover:bg-amber-700 text-white font-semibold text-xs px-4 py-2.5 rounded-xl transition-all font-mono hover:scale-[1.02] shadow-md shadow-amber-600/10"
        >
      ...
      {days.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2.5">
              <AnimatePresence mode="popLayout">
                {activeTasks.length === 0 ? (
                  <p className="text-xs text-slate-650 font-medium">All tooling tasks logged and practiced!</p>
                ) : (
                  activeTasks.map((task) => (
                    <motion.div
                      key={task.id}
                      layoutId={`tool-${task.id}`}
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <button
                          ...
                          className="w-5 h-5 rounded-md border border-slate-800 hover:border-amber-500/80 cursor-pointer flex items-center justify-center transition-colors shrink-0"
                        >
                          <div className="w-3 h-3 rounded bg-transparent group-hover:bg-amber-600/30 transition-colors" />
                        </button>
                        <span className="text-xs text-slate-200 font-semibold truncate select-none">{task.name}</span>
                      </div>
                      <button
                        onClick={() => onDeleteDay(task.id)}
          ...
          <div className="bg-[#111827]/60 rounded-[24px] p-6 border border-white/[0.02] space-y-4">
            <span className="text-[10px] font-mono font-bold tracking-widest text-slate-500 uppercase">
              PRACTICED TOOLS ({completedTasks.length})
            </span>
            <div className="space-y-2.5">
              <AnimatePresence mode="popLayout">
                {completedTasks.length === 0 ? (
                  <p className="text-xs text-slate-650 font-medium">No tools practiced yet.</p>
                ) : (
                  completedTasks.map((task) => (
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <button
                          onClick={() => handleToggleComplete(task)}
                          className="w-5 h-5 rounded-md bg-amber-600/20 border border-amber-500/40 cursor-pointer flex items-center justify-center shrink-0"
                        >
                          <CheckCircle2 className="h-4.5 w-4.5 text-amber-500 animate-pulse" />
                        </button>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    required
                    autoFocus
                    placeholder="e.g. ChatGPT API integration"
                    value={taskName}
                    onChange={(e) => setTaskName(e.target.value)}
                    className="w-full bg-slate-950 border border-slate-900 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-amber-500/80 transition-all font-sans"
                  />
                </div>
                ...
                <div className="flex gap-2.5 pt-2">
                  ...
                  <button
                    type="submit"
                    className="flex-1 bg-amber-600 hover:bg-amber-750 text-white font-semibold py-2.5 rounded-xl text-xs transition-all font-mono shadow-md shadow-amber-600/10"
                  >
                    COMMIT
                  </button>
                </div>
