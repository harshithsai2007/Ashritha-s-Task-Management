...
import { Rocket, Plus, X, Trash2, CheckCircle2, AlertTriangle } from "lucide-react";
...
export default function ProjectModule({ milestones, onUpdateMilestone, onAddMilestone, onDeleteMilestone, title, description, icon: CustomIcon }: ProjectModuleProps) {
  ...
  const handleToggleComplete = (milestone: ProjectMilestone) => {
    const nextVal = !milestone.isCompleted;
    onUpdateMilestone({
      ...milestone,
      isCompleted: nextVal,
  ...
  const activeTasks = milestones.filter((m) => !m.isCompleted);
  const completedTasks = milestones.filter((m) => m.isCompleted);

  return (
    <div className="space-y-6" id="project-module-panel">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-900/40 p-6 rounded-2xl border border-slate-800">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-cyan-600/10 border border-cyan-500/20 flex items-center justify-center text-cyan-400">
            {CustomIcon ? <CustomIcon className="h-6 w-6" /> : <Rocket className="h-6 w-6" />}
          </div>
          <div>
        ...
        <button
          ...
          className="flex items-center gap-2 bg-cyan-600 hover:bg-cyan-700 text-white font-semibold text-xs px-4 py-2.5 rounded-xl transition-all font-mono hover:scale-[1.02] shadow-md shadow-cyan-600/10"
        >
      ...
      {milestones.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center p-16 bg-[#111827] rounded-[24px] border border-white/5 text-center space-y-3">
          <div className="w-12 h-12 rounded-full bg-slate-800/50 flex items-center justify-center text-slate-600">
            {CustomIcon ? <CustomIcon className="h-5 w-5" /> : <Rocket className="h-5 w-5" />}
          </div>
          <span className="text-sm font-semibold text-slate-450">No tasks added yet.</span>
          <p className="text-xs text-slate-500 max-w-xs">Create your very first AI Project task to begin tracking your building milestone steps.</p>
        </div>
      ) : (
        /* Main Grid */
            ...
            <div className="space-y-2.5">
              <AnimatePresence mode="popLayout">
                {activeTasks.length === 0 ? (
                  <p className="text-xs text-slate-650 font-medium">All project tasks completed!</p>
                ) : (
                  activeTasks.map((task) => (
                    <motion.div
                      key={task.id}
                      layoutId={`milestone-${task.id}`}
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <button
                          ...
                          className="w-5 h-5 rounded-md border border-slate-800 hover:border-cyan-500/80 cursor-pointer flex items-center justify-center transition-colors shrink-0"
                        >
                          <div className="w-3 h-3 rounded bg-transparent group-hover:bg-cyan-600/30 transition-colors" />
                        </button>
            ...
            <div className="space-y-2.5">
              <AnimatePresence mode="popLayout">
                {completedTasks.length === 0 ? (
                  <p className="text-xs text-slate-650 font-medium">No tasks completed yet.</p>
                ) : (
                  completedTasks.map((task) => (
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <button
                          onClick={() => handleToggleComplete(task)}
                          className="w-5 h-5 rounded-md bg-cyan-600/20 border border-cyan-500/40 cursor-pointer flex items-center justify-center shrink-0"
                        >
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex items-center gap-2.5 mb-4">
                {CustomIcon ? <CustomIcon className="h-5 w-5 text-cyan-400" /> : <Rocket className="h-5 w-5 text-cyan-400" />}
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-900 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-cyan-500/80 transition-all font-sans"
                  />
                </div>
                ...
                <div className="flex gap-2.5 pt-2">
                  <button
                    type="button"
                    onClick={() => setIsAdding(false)}
                    className="flex-1 border border-slate-900 hover:bg-slate-950 text-slate-400 font-semibold py-2.5 rounded-xl text-xs transition-all font-mono"
                  >
                    CANCEL
                  </button>
                  <button
                    type="submit"
                    className="flex-1 bg-cyan-600 hover:bg-cyan-700 text-white font-semibold py-2.5 rounded-xl text-xs transition-all font-mono shadow-md shadow-cyan-600/10"
                  >
                    COMMIT
                  </button>
                </div>
