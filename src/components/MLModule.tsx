...
import { Brain, Plus, X, Trash2, CheckCircle2 } from "lucide-react";

interface MLModuleProps {
  topics: MLTopic[];
...
export default function MLModule({ topics, onUpdateTopic, onAddTopic, onDeleteTopic }: MLModuleProps) {
  ...
  const handleToggleComplete = (topic: MLTopic) => {
  ...
  return (
    <div className="space-y-6" id="ml-module-container">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-900/40 p-6 rounded-2xl border border-slate-800">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs px-4 py-2.5 rounded-xl transition-all font-mono hover:scale-[1.02] shadow-md shadow-indigo-600/10"
        >
            ...
            <div className="space-y-2.5">
              <AnimatePresence mode="popLayout">
                {activeTasks.length === 0 ? (
                  <p className="text-xs text-slate-650 font-medium">All tasks cleared. Good work!</p>
                ) : (
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <button
                          ...
                          className="w-5 h-5 rounded-md border border-slate-800 hover:border-indigo-500/80 cursor-pointer flex items-center justify-center transition-colors shrink-0"
                        >
                          <div className="w-3 h-3 rounded bg-transparent group-hover:bg-indigo-600/30 transition-colors" />
                        </button>
                        <span className="text-xs text-slate-200 font-semibold truncate select-none">{task.name}</span>
                      </div>
                      <button
                        onClick={() => onDeleteTopic(task.id)}
                  ...
                  completedTasks.map((task) => (
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <button
                          onClick={() => handleToggleComplete(task)}
                          className="w-5 h-5 rounded-md bg-indigo-600/20 border border-indigo-500/40 cursor-pointer flex items-center justify-center shrink-0"
                        >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-900 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500/80 transition-all font-sans"
                  />
                </div>
                ...
                <div className="flex gap-2.5 pt-2">
                  ...
                  <button
                    type="submit"
                    className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2.5 rounded-xl text-xs transition-all font-mono shadow-md shadow-indigo-600/10"
                  >
                    COMMIT
                  </button>
                </div>
